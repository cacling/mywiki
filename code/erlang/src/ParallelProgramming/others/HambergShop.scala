...
object HambergShop {
    ...
    def act() {
      loop {
        receive {
          ...
          case (cookerPID: Actor, "hasHamberg") => println("Waiter: Hi! Customers, we got some new Hambergs!")
  ...
  class HambergFifo extends Actor {
    val hambergs = new Stack[Hamberg]
    val maxSize = 10
    def act() {
      loop {
        receive {
  ...
  class Customer(name: String, hambergFifoPID: Actor, waiterPID: Actor) extends Actor {
    ...
    def act() {
      loop {
        ...
        receive {
          ...
          case (waiter: Actor, "pleaseWait") => println(name + ": OK, Waiting for new hambergs"); Thread.sleep(1000)
          case (waiter: Actor, "noHamberg") => whenNoHamberg()
          case other => println(self + " has received unexpected message " + other)
        }
      }
    }
    ...
    def eatHamberg(hamberg: Hamberg) {
      ...
      val sleeptime = Math.abs(r.nextInt(3000)) * 1
