...
public class DiningPhilosophers {
	...
	class SpaceVehicle extends Thread {
		...
		public SpaceVehicle(int num, SFSController controller, long startTime) {
			...
			System.out.println("SpaceVehicle " + getName() + " supply: "
					+ supply + "; supplyAmount: " + supplyAmount
			...
			while (true) {
				...
				if (supply) {
					...
					System.out.println(System.currentTimeMillis() - startTime
							...
							+ FuelSpaceStation.KIND[supplyKind] + " in its "
							+ j + "-th trip");
				}
				...
				System.out.println(System.currentTimeMillis() - startTime
						...
						+ " and " + amount[1] + " liters of "
						+ FuelSpaceStation.KIND[1] + " in its " + j
						+ "-th trip");
				if (amount[0] == 0 || amount[1] == 0) {
					controller.requestFuel(amount);
					Sleep((int) (1000 * java.lang.Math.random())); // gets the
																	// fuel of
																	// the kind
				} else {
					kind = controller.requestFuel(amount);
					Sleep((int) (1000 * java.lang.Math.random())); // get the
																	// fuel of
																	// the kind
					if (kind != FuelSpaceStation.BOTH) {
						...
						Sleep((int) (1000 * java.lang.Math.random())); // get
																		// another
																		// fluid;
					}
				}
	...
	class SFSController {
		...
		public synchronized void depositFuel(int amount, int kind) { // one at a
																		// time
			...
			fuel[kind] += amount; // depostit the fuel, to avoid race conditions
									// do it here
			notifyAll();
		}
	...
	public class FuelSpaceStation {
		public static final String USAGE = "java FuelSpaceStation <numVehicles> <Ncapacity> <Qcapacity> <numPlaces>";
		public static final int NITROGEN = 0;
		public static final int QUANTUMFLUID = 1;
		public static final int BOTH = 2;
		public static final String[] KIND = { "Nitrogen", "Quantum Fluid" };
		...
		public FuelSpaceStation(String[] args) {
			...
			for (int i = 0; i < numVehicles; i++) {
				vehicle[i] = new SpaceVehicle(i, stationController, startTime);
				vehicle[i].start();
			}
