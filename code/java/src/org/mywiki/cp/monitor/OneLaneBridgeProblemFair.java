...
class Car extends Thread {
  ...
  public Car(String name, int heading, int trips, NS_BridgeController controller, long start) {
    super(name);
  ...
  public void run() {
    for (int j = 1; j <= trips; j++) {
      if (heading == Bridge.HEADINGNORTH) { // heading north
...
public class Bridge {
  ...
  public Bridge(String[] args) {
    ...
    try {
      ...
      if (args.length > 3) atMostCars = Integer.parseInt(args[3]);
      ...
      NS_BridgeController bridgeController = new NS_BridgeController(atMostCars);
      Car[] car = new Car[numCars];
      long start = System.currentTimeMillis();
      for (int i = 0; i < northCars; i++) {
        car[i] = new Car(String.valueOf(i), HEADINGNORTH, trips, bridgeController, start);
        car[i].start();
      }
      for (int i = northCars; i < numCars; i++) {
        car[i] = new Car(String.valueOf(i), HEADINGSOUTH, trips, bridgeController, start);
        car[i].start();
      }
